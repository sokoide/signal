...
10 個の段階的なサンプル（番号順に読むと体系的な理解が得られます）と、発展的な番外編サンプル（`9x_` 番台）。
...
特定の落とし穴を 1 つ深く掘り下げるサンプルです。`make` でビルドされますが、`make run` では自動実行**されません**（停止・ハング・対話が必要なため）。手動で `timeout` を付けて実行してください。

| #  | ファイル              | 学べること                                                                | キーワード                          |
|----|-----------------------|---------------------------------------------------------------------------|-------------------------------------|
| 91 | `91_printf_deadlock`  | **シグナルハンドラ内 `printf` が本当のデッドロックを起こす唯一の条件を実証** | stdio ロック, pthread, デッドロック |
...
### 91_printf_deadlock — ハンドラ内 `printf` が本当にデッドロックする条件を実証

> **注意**: このサンプルは**ハングして永久に戻りません**。`make` でビルドされますが `make run` の対象外です。必ず `timeout` を付けて手動実行してください。
>
> ```sh
> make 91_printf_deadlock        # ビルドのみ
> timeout 5 ./91_printf_deadlock # 実行（5 秒で強制終了）
> ```
>
> **何を見るか**:
> - `main printing 0,1,2` まで表示された後、`>>>` のアナウンスを出して **出力が完全に止まる** — これが `main` と `handler` が互いのロックを待ち合う循環デッドロックで**両スレッドが停止**した証拠。
> - 両スレッドとも進まないため、**Ctrl-C を何度押してもプロセスは落ちません**。`timeout`（または `kill -KILL <pid>`）で落としてください。
> - デッドロックしない安全な書き方（async-signal-safe な `write` の使用）は、直後の [安全に書く方法](#安全に書く方法) と [10_signal_safety](#10_signal_safety--非同期シグナル安全) を参照してください。

「ハンドラ内で `printf` を呼ぶとデッドロックする」とよく言われますが、**単一スレッド**で起こそうとすると環境で結果が変わる罠があります（詳しくは直後の [難所](#なぜハンドラ内-printf-はハングするのか-仕組みの詳細) で）。このサンプルは glibc / macOS libsystem の**両方で確実に**デッドロックを再現し、しかも **main も完全に停止する**目に見えるデッドロックを作ります。

鍵は 2 つ:
1. 「**ハンドラを `main` とは別のスレッドで走らせる**」— `main` 側で SIGINT をブロックして専用の受信スレッドを立て、`kill(getpid(), SIGINT)` が必ずそちらへ配送されるようにする（再帰ロックでも別スレッドからの取得は待たされるため、両環境で確実に詰まる）。
2. 「**main も停止させるための循環待ち**」— `printf` の stdio ロックだけだと片方向の待ちになり main は動き続けてしまう。そこで第2のロック `g_lock` を導入し、main と handler が **逆順** にロックを取得する循環待ち（circular wait）を作る。

```c
/* handler（別スレッドで走る）: g_lock(B) を取得してから printf（= stdout ロック A 取得） */
static void sigint_handler(int sig) {
    pthread_mutex_lock(&g_lock);               /* B を取得 */
    printf("[handler] ... -> DEADLOCK\n");     /* A を待つ → main が持ってる → BLOCK */
    fflush(stdout);
...
```c
/* main: flockfile で stdout(A) を保持 → 数行 printf → SIGINT 送信 → g_lock(B) を待つ */
flockfile(stdout);                              /* A を取得 */
for (int i = 0; i < 3; i++) { printf("main printing %d\n", i); ... }
kill(getpid(), SIGINT);                         /* handler 起動（B を取得させる）*/
pthread_mutex_lock(&g_lock);                    /* B を待つ → handler が持ってる → ここで main も停止 */
```

**このプログラムの構成**:
- `main` 側で `pthread_sigmask(SIG_BLOCK, {SIGINT})` し、専用の**受信スレッド**を立てることで、`kill(getpid(), SIGINT)` は必ず受信スレッド側に配送されます（＝ロック保持者とは**別スレッド**でハンドラが走る）。
- `main` は `flockfile(stdout)` で **A** を保持、ハンドラは `pthread_mutex_lock(&g_lock)` で **B** を保持した状態で、それぞれ相手のロックを取得しようとします。
- 両者が逆順にロックを取得しようとするため **循環待ち（circular wait）** が成立し、どちらのスレッドも進まなくなります。

> **`g_lock` の役割**: デッドロックの本体はあくまで `printf` が内部で取る stdio ロックです。`g_lock` は main を循環待ちに引き込み、**両スレッドが完全に停止する見えるデッドロック**を作るための補助ロックです。stdio ロックだけでも handler は確実に詰まりますが、そのだけだと main は動き続けてしまい「ハング」が目に見えません。

デッドロックを回避するには、ハンドラ内で `printf` を呼ばず、async-signal-safe な `write()` などを使います（[安全に書く方法](#安全に書く方法) 参照）。詳しい仕組みは [難所: なぜハンドラ内 printf はハングするのか](#なぜハンドラ内-printf-はハングするのか-仕組みの詳細) を参照してください。
...
> **正確な注意書き**: 上の「ステップ4でデッドロック」という説明は**直感的な最小例**としては分かりやすいのですが、実は**単一スレッド**のこの構成では結果が libc 実装に依存します — **macOS ではフリーズ**しますが、**Linux glibc では再帰ロックのおかげで通り抜けてしまい死にません**（詳しくは直後の [仕組みの詳細](#なぜハンドラ内-printf-はハングするのか-仕組みの詳細) で）。両プラットフォームで確実に死なせるには「**ロック保持スレッドとは別のスレッドでハンドラを走らせる**」必要があり、それを意図的に再現したのが [`91_printf_deadlock`](#91_printf_deadlock--ハンドラ内-printf-が本当にデッドロックする条件を実証) です。

#### なぜハンドラ内 `printf` はハングするのか — 仕組みの詳細

シグナルハンドラ内で `printf` を呼ぶとハング（デッドロック）する、とよく言われます。しかしこれは**条件次第**です。「本当に無限ブロックする」ための正確なメカニズムを順に追います。

**(1) `printf` は内部でロックを取る**

`printf` は `stdout`（`FILE` 構造体）のバッファを触るため、毎回 `stdout` の**FILE ロック**（`flockfile`/`funlockfile` 相当）を取得・解放します。`malloc` も同様に独自のヒープロックを取ります。これらのロックは async-signal-safe ではありません。

**(2) 単一スレッド・同期的 `raise()` では“実は”止まらない（環境による）**

本教材の `01_signal_basics` のように「`fflush(stdout)` の直後に `raise(SIGINT)`」する構成ではデッドロックしません。理由は:

- **同期的**: `raise()` は `main` が `printf` の外にいる瞬間にハンドラを呼ぶため、そもそもロックを競合しません。

では「`main` の `printf` 実行中（ロック保持中）に非同期シグナルで割り込む」と単一スレッドで試すとどうなるか — **ここが libc 実装で分かれます**:

| libc | FILE ロックの性質 | 単一スレッドでの再入 | 結果 |
|------|-------------------|----------------------|------|
| **glibc (Linux)** | **再帰（リエントラント）** | 同じスレッドからの再取得を**カウント方式で許可** | **死なない**（ハンドラの printf は通って戻る） |
| **macOS libsystem** | **非再帰** | 同じスレッドでも再取得しようとすると**自分が離すのを待って永久ブロック** | **死ぬ**（自己デッドロック） |

> **実測値（本教材の検証で確認）**: ユーザーから提示された「`main` で `printf` を回しながら Ctrl-C 連打」する単一スレッド構成は、**macOS では一瞬でフリーズ**（CPU 0%、出力停止、lldb で `flockfile → __psynch_mutexwait` で永眠を確認）しますが、**Linux glibc では延々動き続け**（CPU 50-76%、数億行/秒で出力継続）ます。glibc は同じスレッドならロックを再入できるため、ハンドラの `printf` が素通りしてしまうのです。

つまり「単一スレッドで printf 実行中にシグナル」は **macOS 専用の再現方法** であり、クロスプラットフォームな教材には使えません。

**(3) 本当のデッドロックが起きる唯一の確実な条件（両プラットフォーム共通）**

真のデッドロックは「**ロックを保持しているスレッド(A)とは別のスレッド(B)** でシグナルハンドラが走り、そこで `printf` を呼ぶ」ときに起きます:

```
スレッド A (main)              スレッド B (受信スレッド)
─────────────────────          ──────────────────────────
printf() ─┐
          │ stdout ロック取得
          │ ...永久に離さない...     [SIGINT 配送 → ハンドラ起動]
          │                         printf() ─┐
          │                                    │ stdout ロックを取得しようとする
          │                                    │ が、A が離さない → 永久に待つ
          └────────────────────       ←────────┘
...
- スレッド A が `stdout` ロックを保持したまま放置し、
- 別スレッド B のハンドラが `printf` → **同じロック**の取得を試みて**永久に待つ**。

ここが核心: **再帰ロックが許す再入は「同じスレッド」だけ**。glibc の FILE ロックが再帰的だとしても、**別スレッド**からの取得は待たされます。だから「`main` とは別のスレッドでハンドラを走らせる」ことが、両プラットフォームで確実にデッドロックさせる唯一の方法です。

> **「片方向の待ち」の性質**: 上の構成では A（main）は動き続け、B（handler）だけが詰まります。そのため `main printing` は出続けますが、`[handler]` は**一度も出力されません**。この「handler がシグナルを受け取ったはずなのに何も出ない」こと自体がデッドロックの証拠です。

**(4) `91_printf_deadlock` が確実に再現する仕組み（main ごと停止させる）**

(3) の「片方向の待ち」だけでも handler は確実に詰まりますが、main は動き続けるため「ハング」が目に見えません。本サンプルは第2のロック `g_lock` を導入し、**main も停止する循環デッドロック**を作ります:

```
  main（スレッドA）              ハンドラ（スレッドB、別スレッドで走る）
  ─────────────────────          ─────────────────────────────────────
  pthread_sigmask(BLOCK SIGINT)  sigsuspend() で SIGINT 待ち
  flockfile(stdout) ─┐ A 保持   ─────────────────────────────────────
  printf("main 0,1,2")│         [SIGINT 配送] → ハンドラ起動
                     │          pthread_mutex_lock(&g_lock) ─┐ B 保持
  kill(SIGINT) ──────┼────────→ printf("[handler]..")
                     │            → 内部で flockfile(stdout) を呼ぶ
                     │            → A を取得しようとするが main が保持
                     │            → 【別スレッド】なので再帰ロックでも再入不可
                     │            → 永久ブロック ★（A 待ち）
  pthread_mutex_lock │   → B を取得しようとする
    (&g_lock)        │     が B は handler が持ってる
                     │     → 永久ブロック ★（B 待ち）
                     └──────────── 互いに待ち合う ──────────┘
                                   (CIRCULAR DEADLOCK)
```
...
1. `main` 側で `pthread_sigmask(SIG_BLOCK, {SIGINT})` し、SIGINT を受け取らないようにする。
2. 専用の**受信スレッド**を立て `sigsuspend` で待機させる（プロセスへの SIGINT はこのスレッドに配送される）。
3. `main` が `flockfile(stdout)` で **A** を保持した状態で数行 `printf` し、動いている様子を見せる。
4. `kill(getpid(), SIGINT)` → 受信スレッド側でハンドラが走り、`pthread_mutex_lock` で **B** を取得してから `printf`（＝ **A** 取得）を試みる → **A** は `main` が持っているので **A 待ちでブロック**。
5. `main` は `pthread_mutex_lock(&g_lock)` で **B** を取得しようとする → **B** はハンドラが持っているので **B 待ちでブロック**。
6. 両者が相手のロックを待ち合う **循環待ち** が成立し、両スレッドとも停止 → `main` の出力も完全に止まる。

> **設計のポイント**: デッドロックの本体は `printf` が内部で取る stdio ロック（handler は `printf` を呼んだ時点で **A 待ち** で詰まる）。`g_lock` は main を循環待ちに引き込み、**両スレッドが完全に停止する見えるデッドロック**を作るための補助ロックです。再帰ロックが「同じスレッドからの再入」しか許さないという性質を利用し、ハンドラを `main` とは**別スレッド**で走らせることで、glibc / macOS 両方で確実にデッドロックさせます。

**(5) 安全な代替**: async-signal-safe な `write(STDOUT_FILENO, ...)` は一切のロックを取らないため、どのスレッド/コンテキストで呼んでもブロックしません（本教材の `safe_helpers.h` の `safe_write_str` / `safe_write_int` がこれにあたります）。
...
| stdio FILE ロックの性質       | **再帰（リエントラント）**: 同じスレッドからの再入をカウント方式で許可 | **非再帰**: 同じスレッドでも再入すると自己デッドロック（91_printf_deadlock 参照） |
...
# 番外編: make run には含まれない（ハング/対話のため手動実行）
make 91_printf_deadlock             # ビルドのみ
timeout 5 ./91_printf_deadlock      # ★意図的デッドロック: 必ず timeout 付きで
...
signal/
  ...
  examples/
    ...
    91_printf_deadlock.c    [番外編] ハンドラ内 printf の真のデッドロックを実証
