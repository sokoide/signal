...
11. [不足トピック補足: EINTR と SIGCHLD](#不足トピック補足-eintr-と-sigchld)
...
**期待される結果**: 間隔が短いほど、実行中のコードの「より近い場所」で割り込まれるため、連続するハンドラ呼び出し間の PC の差が小さくなる。長い間隔では、メインループが大きく進んでから割り込まれるため、PC の差が大きくなる。

### 4. `01_signal_basics` — バックグラウンド起動 + 別ターミナルから `kill`

**やること**: ターミナル A で `./01_signal_basics &` としてバックグラウンド起動し、プロセス ID を確認する。ターミナル B から以下を順に実行する。

```sh
kill -USR1 <PID>
kill -INT <PID>
kill -TERM <PID>
kill -KILL <PID>   # 最後に強制終了
```

**観察すべきこと**: 各シグナルに対してハンドラが呼ばれるか、プロセスが終了するか。

**期待される結果**: `SIGUSR1`/`SIGINT`/`SIGTERM` はキャッチ可能（ただし 01 のコードによっては最後の `SIGTERM` で終了する場合もある）。`SIGKILL` はキャッチ不可・ブロック不可で、プロセスは直ちに終了する。
