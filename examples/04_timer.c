...
int main(void) {
     ...
     *
     * 【発展】今回は「alarm してから pause する」単純な順序なので競合は
     * 起きませんが、一般に「条件をチェックしてから待つ」コードでは
     * チェック後・待機前にシグナルが届くと取りこぼします（古典的な競合）。
     * 本格には sigsuspend(2) を使い、シグナルマスクの変更と待機を
     * 原子的に行ってこの競合を回避します（→ README「発展トピック」）。
     */
    pause();
